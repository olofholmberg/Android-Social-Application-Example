...
import com.example.tddd80application.databinding.FragmentHomeBinding
...
class HomeFragment : BaseFragment<HomeViewModel, FragmentHomeBinding>(), OnQuestionItemClickListener {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        binding.fragmentHomeQuestionsRecyclerView.apply {
            layoutManager = viewManager
            adapter = viewAdapter
        }

        // Update dataset when new set of questions are fetched
        homeViewModel.questions.observe(viewLifecycleOwner, Observer {
            mUserFeedQuestionRecyclerAdapter.updateData(it.questions)
        })

        homeViewModel.apiErrorMessage.observe(viewLifecycleOwner, Observer {
            Toast.makeText(requireContext(), it.msg, Toast.LENGTH_LONG).show()
        })

        // Add ask question button listener
        binding.fragmentHomeAskQuestionButton.setOnClickListener {
        ...
        lifecycleScope.launch {
            ...
            if (accessToken != null && accessToken.isNotEmpty()) {
                homeViewModel.fetchQuestions(accessToken)
            }
        }
    }
